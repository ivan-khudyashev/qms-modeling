...
def CDF_draw_test(cdf_f, cnt_test, common_title):
    ...
    x_max = plot_core.find_x_max(cdf_f)
    x_cdf, y_cdf = plot_core.tabulate_CDF_by_randCDF(rand_exp, cnt_test, True, x_max, 1.0)
    x_pdf, y_pdf = plot_core.tabulate_CDF_by_randCDF(rand_exp, cnt_test, False, x_max, 1.0)
    plots = [{"x": x_cdf, "y": y_cdf, "title": "CDF"}, {"x": x_pdf, "y": y_pdf, "title": "PDF"}]
    plot_core.draw_plots(plots, common_title)
